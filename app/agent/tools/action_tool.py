"""Thu vien hanh dong D3 - danh gia dieu kien du va tra ve cac hanh dong da
kiem chung. LLM khong duoc phep tu nghi hanh dong, chi CHON trong so cac
hanh dong DA DU DIEU KIEN va viet loi giai thich (CLAUDE.md). Xem
docs/06-agent-design.md muc 6.7.
...
def evaluate_eligibility(
    ...
    """Dispatch RO RANG theo tung action.id - tranh loi so luong tham so
    khi cac ham kiem tra co chu ky khac nhau (mot so can `segments`, mot so
    can `runner`, mot so can ca hai)."""
...
def list_actions_with_eligibility(
    runner: SqlMetricRunner, segments: list[dict[str, Any]] | None = None,
) -> list[dict[str, Any]]:
    """Danh sach 6 hanh dong D3, sap theo priority, kem trang thai du dieu
    kien va tac dong da kiem chung. `segments` truyen san de tranh tinh lai
    (agent thuong da co san bang phan khuc tu buoc COMPUTE)."""
    from app.agent.tools.segment_tool import build_segment_table

    cfg = get_analytics_config()
    segments = segments if segments is not None else build_segment_table()

    out: list[dict[str, Any]] = []
    for action in sorted(cfg.actions, key=lambda a: a.priority):
        eligible, note = evaluate_eligibility(action, runner, segments)
        out.append({
            "id": action.id,
...
__all__ = ["list_actions_with_eligibility", "evaluate_eligibility"]
